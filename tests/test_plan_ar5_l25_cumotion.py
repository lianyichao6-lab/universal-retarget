...
def test_hand_timeline_inserts_close_before_lift() -> None:
    hand, phase = _hand_timeline(
        np.asarray(["pregrasp", "approach", "close", "close", "close", "lift"]),
        np.ones(21, dtype=np.float32),
        np.zeros(16, dtype=np.float32),
        close_frames=3,
    )
    assert hand.shape == (6, 21)
    assert phase.tolist() == ["pregrasp", "approach", "close", "close", "close", "lift"]
    np.testing.assert_allclose(hand[-1], 1.0)
