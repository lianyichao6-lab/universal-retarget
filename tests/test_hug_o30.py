...
def test_hug_o30_retarget_produces_a_finite_controller_command() -> None:
    ...
    assert result.luban_active_positions.shape == (20,)
    assert np.isfinite(result.qpos).all()
    assert np.isfinite(result.luban_active_positions).all()
    assert result.qpos_joint_names == O30_QPOS_JOINT_NAMES
    assert result.luban_active_joint_names == O30_ACTIVE_JOINT_NAMES
