#!/usr/bin/env python3
"""Freeze a capture-time AnyDex grasp plan into a Luban planning request."""
...
def main() -> None:
    ...
    parser.add_argument("--lift-direction-base", type=float, nargs=3, default=(0.0, 0.0, 1.0), metavar=("X", "Y", "Z"))
    ...
    request = build_luban_grasp_request(
        ...
        lift_direction_base=args.lift_direction_base,
    )
    args.output.parent.mkdir(parents=True, exist_ok=True)
    np.savez_compressed(args.output, **request)
    report = {key: (value.item() if value.shape == () else value.tolist()) for key, value in request.items()}
    report["safety"] = "Planning only. Arm and hand commands are not published."
    args.output.with_suffix(".json").write_text(json.dumps(report, indent=2) + "\n", encoding="utf-8")
    print(f"Luban grasp request written: {args.output}")
    print("  L25 command: 16 active joints in radians")
    print("  arm targets: pregrasp, grasp and lift poses in robot base frame")
