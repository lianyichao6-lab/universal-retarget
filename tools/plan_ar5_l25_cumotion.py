...
def _hand_timeline(
    ...
    for value in phase:
        ...
        if name == "pregrasp":
            ...
            close_index += 1
        elif name in ("close_hold", "lift", "lift_hold"):
...
def build_plan(
    ...
    *,
    timeout: float,
    close_frames: int,
    close_hold_frames: int = 10,
    lift_hold_frames: int = 10,
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    if close_frames < 1 or close_hold_frames < 0 or lift_hold_frames < 0:
        raise ValueError("close_frames must be positive and hold frames must be non-negative")
    ...
    if close_hold_frames:
        arm_parts.append(np.repeat(current[None], close_hold_frames, axis=0))
        phase_parts.append(np.full(close_hold_frames, "close_hold"))
    lift = _query_segment(
        current,
    ...
    current = lift[-1]
    if lift_hold_frames:
        arm_parts.append(np.repeat(current[None], lift_hold_frames, axis=0))
        phase_parts.append(np.full(lift_hold_frames, "lift_hold"))
...
def main() -> None:
    ...
    if args.fps <= 0.0 or args.timeout <= 0.0 or args.close_frames < 1 or args.close_hold_frames < 0 or args.lift_hold_frames < 0:
        parser.error("fps, timeout and close-frames must be positive; hold frames must be non-negative")
    ...
    arm, phase, hand, hand_phase = build_plan(
        ...
        close_hold_frames=args.close_hold_frames,
        lift_hold_frames=args.lift_hold_frames,
    )
