...
The output is compatible with ``tools/luban_hand_ros_execute.py --hand-model o30``.
...
def main() -> None:
    ...
    np.savez_compressed(
        ...
        luban_hand_positions=np.repeat(result.luban_active_positions[None], args.frames, axis=0),
        luban_hand_joint_names=np.asarray(O30_ACTIVE_JOINT_NAMES),
    )
    metadata = {
