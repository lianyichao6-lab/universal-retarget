...
def _load_arm_trajectory(path: Path) -> tuple[np.ndarray, np.ndarray | None]:
    ...
    with np.load(path, allow_pickle=False) as payload:
        ...
        positions_key = next((key for key in ("positions", "arm_positions", "ar5_qpos") if key in payload), None)
        if positions_key is None:
            raise ValueError("arm trajectory requires positions, arm_positions, or ar5_qpos")
        names = AR5_RIGHT_JOINT_NAMES if names_key is None else tuple(str(name) for name in np.asarray(payload[names_key]).tolist())
...
def _load_hand_episode(path: Path) -> tuple[np.ndarray, np.ndarray | None, np.ndarray | None]:
    ...
    with np.load(path, allow_pickle=False) as payload:
        qpos_key = "qpos" if "qpos" in payload else "l25_qpos" if "l25_qpos" in payload else None
        if qpos_key is None:
            raise ValueError("hand episode requires qpos or l25_qpos")
        qpos = np.asarray(payload[qpos_key], dtype=np.float64)
        timestamps = np.asarray(payload["timestamps"], dtype=np.float64) if "timestamps" in payload else None
...
def apply_offline_frame(model, data, addresses: dict[str, int], arm: np.ndarray, hand: np.ndarray, object_positions: np.ndarray | None, index: int) -> None:
    ...
    hand_names = tuple(
        "r_hand_thumb_dip" if name == "thumb_ip" and "r_hand_thumb_dip" in addresses else f"r_hand_{name}"
        for name in L25_QPOS_JOINTS
    )
    apply_joint_state(model, data, addresses, hand_names, hand[index])
