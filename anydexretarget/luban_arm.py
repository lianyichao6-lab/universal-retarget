...
def arm_flange_target(
    ...
    *,
    ...
    hand_pose_key: str = "T_anchor_l25_hand",
) -> np.ndarray:
    """Compute ``T_robot_base_arm_flange`` from a grasp contract and calibration."""
    if hand_pose_key not in grasp_contract:
        raise ValueError(f"grasp contract is missing {hand_pose_key}")
    t_anchor_l25_hand = homogeneous_transform(
        grasp_contract[hand_pose_key], hand_pose_key
    )
